...
export const SnapGuideOverlay = ({ camera, moveInteraction }: SnapGuideOverlayProps) => {
  ...
  useLayoutEffect(() => {
    ...
    const apply = (): void => {
      ...
      const guides = snapshot.kind === 'moving' ? snapshot.guides : [];
  ...
  }, [camera, moveInteraction]);

  return (
    <g data-guide-count="0" data-snap-guide-overlay="move-guides" display="none" ref={groupRef}>
      {SNAP_AXES.map((axis) => (
