...
| D-020 | Signed public-release updates through Electron's service   | Reuse the supported Squirrel feed for the public repository; draft releases are the rollout boundary and normal quit preserves save/recovery authority | Accepted                                  |
| D-021 | Separate macOS arm64 and x64 release artifacts             | Avoid making every user download a roughly 75% larger universal Electron bundle while retaining one source and identical release gates                 | Accepted                                  |
