...
- The former narrow alpha spec is now an explicit declarative `ControlDefinition` boundary. Its current contract owns stable type and file version, validated defaults/properties, default/minimum/optional maximum size, optional auto-size policy, generic capability metadata, palette and search metadata, inspector property fields, scene primitive/property dependencies, exact hit shape, accessibility semantics, and a sequential property-migration chain without importing React, DOM, Electron, or platform code into the domain. Registry construction fails atomically for duplicate types, invalid sizes/defaults, missing property references, invalid capability/accessibility/hit metadata, invalid primitive geometry, duplicate palette order, or incomplete migrations. Thumbnail/export contracts and non-v1 migration execution remain unchecked M8 work; migration execution is not safe until persisted element data carries a durable source control/property version.
...
The next unchecked M8 foundation seam is the definition-owned thumbnail/export contract. Non-v1 migration execution remains a dedicated compatible slice because persisted element data currently has no durable source control/property version. After those seams, add the screenshot-verified representative Image Placeholder, Browser/container, and Arrow controls through the same registry. That catalog/control surface is handed to the parallel Layout task; this branch must not add element-specific types or inspector fields without a new ownership handoff. The visible icon picker and broad catalog remain unchecked even though the curated outline catalog is prepared. Canonical scene baselines now unblock a later disposable baseline-snap adapter, but M7 remains intentionally paused until the roadmap reactivates it or a concrete M8 workflow blocker is documented.
