...
- Fifty-nine focused viewport cases include 10,000 deterministic seeded coordinate round trips, framing and resize invariants, 1,000 raw camera updates coalesced into one publication, cursor-anchor preservation, input-unit normalization, cancel-safe pointer paths, deterministic sketch goldens, spatial-index failure/oversize cases, and fixed 1,000/5,000-element source-level performance fixtures. The full local 51-file/295-test source gate, package, fuse readback, ordinary smoke, nine-state packaged visual matrix, create/edit/save/close/reopen workflow, and forced-crash ordinary-launch recovery all pass; the native screenshot was reviewed with the scene contained by the unchanged shell. The first camera/scene slice also passed [push run 31833568347](https://github.com/Honigstute/Balisami/actions/runs/31833568347) and independent [PR run 31833593703](https://github.com/Honigstute/Balisami/actions/runs/31833593703) on native macOS 26 arm64 and Windows 2025 x64.
...
Add one narrow document-to-scene adapter for the active board and render the existing foundation rectangles through the seeded path primitive. Build the spatial index from derived world bounds, cull through the visible-range contract, update only changed element revisions, and retain canonical `childIds` stacking; do not introduce selection or component-specific editor behavior yet.
