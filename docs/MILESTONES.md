...
- Bind bounded recovery discovery and Open/recent transitions to the renderer with an explicit restore/discard/current-project decision so ordinary startup—not only the isolated verifier—can recover a crash without guessing or silently replacing work. Keep documents opaque across the shared contract, preserve the one renderer history authority during project replacement, and add packaged crash→ordinary-launch recovery acceptance on both platforms. Re-audit every M3 deliverable before changing milestone status.
