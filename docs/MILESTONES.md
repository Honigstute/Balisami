...
- Run the 31-file/174-test source gate and both packaged persistence probes on native macOS and Windows hosts, then record the exact evidence. M3 remains active after this integration slice: bind bounded recovery discovery and Open/recent transitions to the renderer with an explicit restore/discard/current-project decision so ordinary startup—not only the isolated verifier—can recover a crash without guessing or silently replacing work. Re-audit every M3 deliverable before changing milestone status.
