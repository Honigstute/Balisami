...
Begin the keyboard-nudge slice without widening into delete, duplicate, or clipboard behavior: define one platform-neutral arrow-key contract at the existing viewport input owner, ignore editable overlays, and reuse move capture so selected descendants, locked controls, and nested local frames keep the proven pointer semantics. Specify keydown/key-repeat/keyup transaction boundaries before implementation, use one world unit normally and ten with Shift independent of zoom/DPR, commit through `ProjectSession`, and prove exact selection-change/blur/teardown cancellation, deterministic coalescing, recovery scheduling, one-step undo, and unrelated-node render isolation before adding the next command family.
