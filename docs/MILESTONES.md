...
Begin the first geometry-writing M6 slice with move only: define a pure transient move preview from immutable original local frames plus current world delta, expose it to the scene and fixed-screen selection overlay without mutating the document per pointer event, and commit one validated history transaction on pointer-up. Prove 500 pointer updates yield one undo entry, Escape/capture loss restore exact geometry and selection, nested local/world coordinates do not drift, unrelated scene nodes do not rerender, and Shift axis-lock behavior is deterministic before adding resize handles.
