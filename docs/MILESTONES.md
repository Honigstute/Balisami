...
- Every fixed shell region has one stable semantic marker. A pure shared geometry contract derives its expected border box from the same tokens used by CSS and native window setup. The packaged smoke test resizes the native Electron window to the exact 1024×680 minimum and then to 1440×900 bounded by the host display's available work area, waits for the platform-specific content viewport to settle, and rejects any anchor error above 0.5 CSS pixel before capturing its screenshot; native title/frame insets never become renderer magic numbers.
...
- A deterministic packaged visual-conformance mode now owns six named states: default/loading, representative controls, bounded failure plus capped notice, tooltip, popover, and modal. Every state launches the production package in a separate process, waits for renderer readiness, proves the same minimum/normal shell geometry contract, rejects stderr or health-monitor failures, and captures a non-empty review artifact. Fixture-only state disables user preference reads so existing pane settings cannot contaminate baselines.
...
Push the M4 packaged visual-conformance matrix, require source/package/fuse/smoke/project-workflow/crash-recovery/visual jobs to pass on native macOS arm64 and Windows x64, then inspect both uploaded six-state screenshot sets before closing M4. Do not begin M5 viewport work until that cross-platform review is recorded.
