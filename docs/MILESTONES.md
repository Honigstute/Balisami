...
- The viewport's existing input controller remains the sole pointer owner. Primary click selection, Shift-toggle, screen-pixel click slop, empty-space clearing, and Escape share one `idle -> pressed -> idle` contract with space/primary and middle-button panning. Pointer completion commits selection once; movement beyond click slop, pointer cancellation, capture loss, window blur, teardown, or Escape discards the press without reconstructing state. Movement is intentionally not promoted to marquee or transform yet.
...
Extend the proven selection contract before introducing geometry writes: promote a moved empty-space press into a transient marquee state, add containment/intersection policy, Shift-marquee, Select All, and deterministic overlap cycling through the existing topmost hit stack. Keep every preview session-only, add an accessible keyboard route to the canvas, and prove cancellation plus selection-scope changes still leave document/history untouched and unrelated scene nodes unrendered. Only then begin the move/resize transaction contract.
