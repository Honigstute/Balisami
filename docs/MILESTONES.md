...
- Focused coverage proves unique shell anchors, default/resized/collapsed center-column geometry, invalid viewport rejection, recovery/startup-error overlays that leave the canvas mounted, two-font and token ownership, modal accessibility/focus behavior, strict preference fallback/persistence, bounded keyboard and pointer resizing, cancellation, and stable button tones. The full local 35-file/205-test source gate, package, fuse readback, geometry-aware smoke, create/edit/save/close/reopen workflow, and forced-crash ordinary-launch recovery all pass; the prior 531-byte user file remains byte-identical.
