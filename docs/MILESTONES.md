...
- Exact idle `Cmd/Ctrl+G` and `Cmd/Ctrl+Shift+G` routes commit once through `ProjectSession` and reconcile selection only from accepted output. A transparent scene container keeps a selected group movable, fit-capable, and available as a container snap source without drawing fake control chrome; the fixed-screen overlay shows its bounds without resize handles. The new deterministic `groupSelection` packaged artifact was reviewed at original resolution with unchanged child drawings and shell tracks.
...
Continue M7 with pure lock/unlock and z-order arrangement contracts before adding more geometry actions. Define direct versus ancestor lock semantics, canonical multi-selection roots, same-owner requirements, exact Back/Front/Backward/Forward order transforms, boundary no-ops, and interaction availability from the same selectors. Prove one accepted action creates one transaction/recovery schedule, undo/redo restores byte-equivalent lock and sibling order under nested randomized fixtures, and locked descendants cannot leak into move/resize/group planners. Only then wire exact platform shortcuts and session selection reconciliation; align/distribute, equal-gap guides, baseline targets, and board targets remain later M7 slices.
