...
Begin the in-session clipboard-command foundation without claiming M12 system/cross-application interoperability: define a small runtime-validated versioned payload for the current childless selection, exact idle `Cmd/Ctrl+C`, `Cmd/Ctrl+X`, and `Cmd/Ctrl+V` mappings, and one session clipboard authority outside document/history state. Copy must not create history or recovery work; cut must prepare the complete payload before one accepted delete transaction and publish it only after that document commit; paste must collision-remap every ID, validate owners and references against the current project, use deterministic ordering/offset policy, commit once, and select only accepted pasted IDs. Prove modifier/editable/repeat/active-gesture isolation, stale/locked/container/owner/collision failure, exact cut/paste undo, one recovery schedule per mutating command, no document persistence of clipboard state, and unchanged scene/shell geometry. Keep OS clipboard formats, cross-project asset/link remapping, and external application fallbacks explicitly in M12.
