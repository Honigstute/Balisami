...
Continue M7 with pure multi-selection align/distribute contracts using the established canonical-root, effective-lock, world/local-coordinate, command-transaction, and accepted-output rules. Define deterministic same-owner requirements, horizontal/vertical anchor semantics, equal-gap behavior, minimum-selection counts, stable reference selection, no-op boundaries, and exact undo/redo before wiring commands or shortcuts. Then extend the shared snap vocabulary with equal-gap, baseline, parent/container-padding, and board/content targets without creating a parallel resolver or weakening the 5,000-element budget.
