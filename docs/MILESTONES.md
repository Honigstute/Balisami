...
Begin the M6 in-place text-edit overlay foundation without prematurely expanding the M8 control registry: define one explicit edit-session authority and injectable text target/commit contract, anchor a fixed-screen DOM editor from canonical world geometry through the existing viewport transform, and keep draft text outside the persisted document until one accepted completion command. Double-click/Enter entry, input composition, selection, platform text shortcuts, camera changes, and overlay focus must remain native and isolated from viewport selection/pan/nudge/delete/duplicate/clipboard commands. Escape must cancel to the byte-equivalent starting document; accepted completion must create one history entry and one recovery schedule; blur and scene/selection replacement need one documented deterministic policy rather than duplicate commits. Prove zoom/DPR anchoring, no React or document work per keystroke beyond the isolated overlay, error/cancellation cleanup, exact undo, unrelated-node render isolation, and unchanged shell tracks in a dedicated packaged visual fixture. Keep actual Text/Button/Input property schemas, measurement, and broad control rendering in M8.
