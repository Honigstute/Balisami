...
- Forty-four focused viewport cases include 10,000 deterministic seeded point/vector/rect round trips across negative and million-unit world coordinates, framing and resize invariants, 1,000 raw camera updates coalesced into one publication, cursor-anchor preservation, input-unit normalization, cancel-safe pointer paths, layer uniqueness, and a proof that camera-only motion leaves world-child render count at one. The full local 47-file/280-test source gate, package, fuse readback, ordinary smoke, nine-state packaged visual matrix, create/edit/save/close/reopen workflow, and forced-crash ordinary-launch recovery all pass; the native screenshot was reviewed with the scene contained by the unchanged shell. The first camera/scene slice also passed [push run 31833568347](https://github.com/Honigstute/Balisami/actions/runs/31833568347) and independent [PR run 31833593703](https://github.com/Honigstute/Balisami/actions/runs/31833593703) on native macOS 26 arm64 and Windows 2025 x64.
...
Define deterministic seeded sketch primitives and pure visible-world-range/spatial-index interfaces, then benchmark their query/update behavior with fixed 1,000- and 5,000-element fixtures. Only after those contracts pass should the first domain rectangles enter the document scene; camera-only motion must still leave individual node render counts unchanged.
