...
Begin M7 with the pure snap-resolution foundation before drawing guides or changing gesture commits. Define validated grid/object/container candidates from canonical world geometry, screen-pixel tolerance converted once by zoom, independent X/Y scoring, deterministic priority/tie rules, acquire/release hysteresis, selected-subtree exclusion, and a modifier bypass contract. Keep resolution free of DOM reads, document mutation, React, and rendering concerns; query nearby candidates through the existing spatial index rather than scanning dense scenes. Prove identical world results across zoom/DPR, negative coordinates, ambiguous ties, threshold jitter, and a 5,000-node p95 budget. Then let move preview consume the resolved delta transiently while pointer-up still produces one existing command transaction; guide descriptors and the fixed-screen overlay follow only after the resolver is stable.
