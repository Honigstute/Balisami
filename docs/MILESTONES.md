...
Continue M7 with the pure grouping/ungrouping domain contract before adding UI shortcuts. Define same-owner canonical selection rules, group-frame union and insertion order, exact owner-local → group-local frame conversion without world movement, inverse ungroup placement, nested ownership, locked/stale/cross-owner rejection, and deterministic IDs. Prove group → undo → redo and group → ungroup preserve byte-equivalent world geometry and sibling stacking under randomized nested fixtures, with one transaction and recovery schedule per accepted user action. Only then wire viewport commands and transient selection reconciliation; arrangement commands and additional snap target classes remain later M7 slices.
