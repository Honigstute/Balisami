...
Begin the keyboard-delete slice without widening into duplicate, clipboard, or text-edit behavior: define one platform-neutral Delete/Backspace contract at the existing viewport input owner, ignore repeats, modified shortcuts, editable overlays, and every active gesture, and derive availability from the current command registry's explicit childless-delete constraint rather than inventing recursive group semantics before M7. Plan the complete selected-ID transaction in deterministic canonical order before mutation, commit once through `ProjectSession`, reconcile session selection only after accepted document change, and prove all-or-nothing failure, exact sibling/selection restoration on one-step undo, one recovery schedule, no action for locked or unavailable selections, and unchanged scene/shell geometry before adding duplicate.
