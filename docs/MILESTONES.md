...
Begin the keyboard-duplicate slice without widening into clipboard, copy/paste, or text-edit behavior: define exact platform shortcut ownership (`Cmd+D` on macOS, `Ctrl+D` on Windows), leave repeats, editable overlays, alternate modifier combinations, and active gestures untouched, and keep the current childless-control constraint rather than inventing subtree cloning before M7. Build a pure all-or-nothing planner with injected collision-checked ID allocation, canonical selected-root ordering, preserved local geometry/properties/links/assets, an explicit deterministic world-unit offset, and deterministic sibling insertion. Commit the clones once through `ProjectSession`, select only accepted clone IDs afterward, and prove stale/locked/container/collision rejection, nested local-coordinate correctness, exact document/order restoration on one undo, one recovery schedule, unrelated-node render isolation, and stable scene/shell geometry.
