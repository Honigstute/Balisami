...
- Unsaved close is an explicit Save/Cancel/Don't Save state machine. Cancel and cancelled Save As keep the session open, Save closes only after durable success, and Don't Save invokes the bounded recovery-discard path. The eventual renderer bridge must freeze or recheck history while the native decision is pending.
...
- Implement the remaining M3 integration slice: one renderer project-session/history authority, a narrow runtime-validated preload/main binding to the completed native workflow, and save/dirty/recovery state in the existing reserved status strip. The window-close handshake must freeze or recheck history across the native decision, and transport tests must prove paths and raw failures stay in main. Then run the packaged create/save/close/reopen gate and audit every M3 deliverable before changing milestone status.
