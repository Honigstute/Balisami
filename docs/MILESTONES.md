...
Begin the single-selection resize slice without changing move semantics: define eight fixed-screen handle hit zones from derived selection bounds, pure opposite-anchor local-frame math with explicit minimum dimensions, and deterministic Shift aspect locking from the immutable starting frame. Route resize through the existing pointer owner and animation-frame preview boundary, commit one validated frame command on pointer-up, and prove 500 updates, zoom/DPR invariance, nested coordinates, every cancellation path, constant handle size, exact undo, and unrelated-node render isolation before attempting multi-selection resize.
