...
- Thirty-four focused viewport cases include 10,000 deterministic seeded point/vector/rect round trips across negative and million-unit world coordinates, framing and resize invariants, 1,000 raw camera updates coalesced into one publication, cursor-anchor preservation, layer uniqueness, and a proof that camera-only motion leaves world-child render count at one. The full local 46-file/270-test source gate, package, fuse readback, ordinary smoke, nine-state packaged visual matrix, create/edit/save/close/reopen workflow, and forced-crash ordinary-launch recovery all pass; the native screenshot was reviewed with the scene contained by the unchanged shell.
...
Define and test the platform-neutral wheel/pinch and pointer-pan normalization contract, then connect pointer-centered zoom and cancellable space/middle-button pan to the camera store. Keep raw-event state outside React, publish at most once per animation frame, and prove cursor anchoring plus resize/pane-toggle stability before adding document nodes.
