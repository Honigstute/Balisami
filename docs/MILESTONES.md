...
- A pure axis-aware auto-size projection consumes measured text, definition-supplied insets, current size, and minimum/optional maximum constraints; it clamps only active axes, preserves inactive canonical dimensions exactly, and rejects non-finite or inverted geometry. One shared text policy now drives persisted property validation, inspector inputs, inline editing, and measurement. Wiring maximum/auto-size metadata into `ControlDefinition`, exposing the inspector action, and committing its projected frame through one validated history command remain intentionally unchecked for the Layout task; this foundation does not add another control type or element-specific inspector field.
...
Hand the active M8 control/catalog surface to the parallel Layout task after it incorporates this `ControlDefinition`, Checkbox conformance, and deterministic text-measurement foundation. The next unchecked foundation item is definition-owned maximum/auto-size metadata: the schema-driven inspector may then call the shared service and commit its projected frame through one validated command. Scene text renderers must consume the same layout/baseline results before paused M7 text-baseline snapping can resume. The Layout task should complete the remaining explicit definition fields and extend the same registry only from screenshot-verified evidence; it must not add palette, scene, inspector, serializer, search, thumbnail, or export type switches. Keep Image Placeholder, Browser/container, Arrow, and the broader catalog unchecked until their evidence and shared primitives are implemented and verified. Resume other paused M7 targets only for a documented workflow blocker.
