...
- Every fixed shell region has one stable semantic marker. A pure shared geometry contract derives its expected border box from the same tokens used by CSS and native window setup. The packaged smoke test resizes the native Electron window to 1024×680 and 1440×900, waits for the platform-specific content viewport to settle, and rejects any anchor error above 0.5 CSS pixel before capturing its screenshot; native title/frame insets never become renderer magic numbers.
...
- Focused coverage proves unique shell anchors, default/resized/collapsed center-column geometry, native-frame-aware viewport sizing, invalid viewport rejection, recovery/startup-error overlays that leave the canvas mounted, two-font and token ownership, modal accessibility/focus behavior, strict preference fallback/persistence, bounded keyboard and pointer resizing/cancellation, regional isolation/retry, capped notice behavior, and stable button tones. The full local 37-file/212-test source gate, package, fuse readback, geometry-aware smoke, create/edit/save/close/reopen workflow, and forced-crash ordinary-launch recovery all pass; the prior 531-byte user file remains byte-identical.
