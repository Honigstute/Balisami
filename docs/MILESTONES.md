...
- Add a packaged forced-crash recovery probe that persists one accepted recovery state, terminates without the close path, relaunches, discovers and validates that exact state, and leaves the prior user file intact on native macOS and Windows. Then bind the completed main-only project workflow to one per-window, runtime-validated IPC/close coordinator when M4 introduces the renderer history authority and reserved save-status slot; do not duplicate document schemas or expose paths to achieve that binding.
