...
Continue M7 by extending the same pure resolver into transient resize preview without changing resize command/history semantics. Define which moving edges and axes each of the eight handles exposes, compose snapping deterministically with the existing Shift aspect/anchor contract, reuse indexed candidate bands and the exact platform-primary bypass, and let the fixed-screen overlay consume the resulting guides. Prove all handles across zoom/DPR, live modifier changes, minimum-size clamping, hysteresis, cancellation/failure, one-command completion, exact undo, and the 5,000-node budget before adding container/grid/baseline/equal-gap targets or arrangement commands.
