...
import {
  ...
  parseProjectFileFixture,
  PROJECT_FILE_FIXTURE_ASSET_BYTES,
  PROJECT_FILE_FIXTURE_ASSET_SHA_256,
...
const encodeFixture = (
  document: ProjectDocument = createAssetFreeProjectDocument(),
...
describe('project file codec', () => {
  ...
  it('stores referenced asset bytes by digest and verifies them on round-trip', () => {
    ...
    const assetPath = getProjectAssetEntryPath(PROJECT_FILE_FIXTURE_ASSET_SHA_256);
    const decoded = decodeProjectFileEnvelope(encoded);

    expect(assetPath).toBe(
      `${PROJECT_FILE_ENTRY_PATHS.assetDirectory}${PROJECT_FILE_FIXTURE_ASSET_SHA_256}`,
    );
    expect(encoded.entries.map((entry) => entry.path)).toEqual([
      PROJECT_FILE_ENTRY_PATHS.manifest,
      PROJECT_FILE_ENTRY_PATHS.document,
  ...
  it('deduplicates identical binary assets without merging their document identities', () => {
    ...
    primaryReference.sha256 = PROJECT_FILE_FIXTURE_ASSET_SHA_256;
    primaryReference.byteLength = PROJECT_FILE_FIXTURE_ASSET_BYTES.byteLength;
    ...
    const encoded = encodeFixture(document, {
      [DOCUMENT_FIXTURE_IDS.asset]: PROJECT_FILE_FIXTURE_ASSET_BYTES,
      [duplicateAssetId]: Uint8Array.from(PROJECT_FILE_FIXTURE_ASSET_BYTES),
    });
    const decoded = decodeProjectFileEnvelope(encoded);
  ...
  it('rejects invalid documents and missing, extra, damaged, or mislabeled assets', () => {
    const assetDocument = createProjectDocumentWithAsset();
    ...
    expect(
      expectDecodeError({
        entries: [
          ...envelope.entries,
          {
            path: getProjectAssetEntryPath(unreferencedDigest),
            bytes: PROJECT_FILE_FIXTURE_ASSET_BYTES,
          },
        ],
      }).code,
  ...
  it('validates assets while encoding and never exposes caller-owned byte arrays', () => {
    ...
    expect(
      encodeProjectFileEnvelope(document, {
        [DOCUMENT_FIXTURE_IDS.asset]: PROJECT_FILE_FIXTURE_ASSET_BYTES,
        asset_unknown01: PROJECT_FILE_FIXTURE_ASSET_BYTES,
      }),
    ).toMatchObject({ ok: false, error: { code: 'unexpected-asset' } });

    const callerBytes = Uint8Array.from(PROJECT_FILE_FIXTURE_ASSET_BYTES);
    ...
    expect(Array.from(assetEntry?.bytes ?? [])).toEqual(
      Array.from(PROJECT_FILE_FIXTURE_ASSET_BYTES),
    );
