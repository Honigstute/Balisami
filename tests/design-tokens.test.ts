...
describe('visual token contract', () => {
  ...
  it('owns the two allowed font families and visual literals centrally', async () => {
    ...
    const tokenPath = path.join(process.cwd(), 'src/shared/design-tokens.ts');
    const sourcePaths = await collectTypeScriptSources(path.join(process.cwd(), 'src'));
    for (const sourcePath of sourcePaths.filter((candidate) => candidate !== tokenPath)) {
      const source = await readFile(sourcePath, 'utf8');
      expect(
        source,
        `${path.relative(process.cwd(), sourcePath)} contains a raw color`,
      ).not.toMatch(/#(?:[0-9a-f]{3}){1,2}\b|\brgba?\(/iu);
    }
  });
});
